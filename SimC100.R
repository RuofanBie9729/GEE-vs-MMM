...
newdata1 = GenBinaryY(mean.formula=~time*Xe, lv.formula=~1, 
          beta=beta, sigma=sigma1, id=id, data=data[which(Xe>0),], q=120, 
          Yname = "binY") # sigma=0.8 for postive exposure
newdata1=rbind(newdata1, 
          GenBinaryY(mean.formula=~time*Xe, lv.formula=~1, 
          beta=beta, sigma=sigma2, id=id, data=data[which(Xe==0),], q=120, 
          Yname = "binY")) # sigma=0.3 for non-positive exposure
