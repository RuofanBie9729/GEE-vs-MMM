...
// I noticed that c code for solving MMM model also used this function, so I changed the
// name of this function to keep normal assumption based estimation.
//
SEXP DeconvolveGL_CALL(SEXP SEXP_eta,  
                           SEXP SEXP_gamma, 
                           SEXP SEXP_lambda, 
                           SEXP SEXP_sigma, 
                           SEXP SEXP_z, 
                           SEXP SEXP_w){ 
//                           double offset ){
// Because the integration includes parameter lambda, I defined lambda in this function.
    double etai, gamma, lambda, sigma, z, w;
    int q_points = 0;
    int n, s, t;
    ...
    double *S_eta, *S_gamma, *S_lambda, *S_sigma, *S_z, *S_w;
    ...
    PROTECT(SEXP_lambda = coerceVector(SEXP_lambda, REALSXP));
    S_lambda = REAL(SEXP_lambda);
    ...
    for (s=0; s<n; s++){
        ...
        do{
            ...
            for(j=0; j<q_points; j++){
				...
				//Based on calculations, I changed the equation to calculate h(delta+b). 
				//From sigma*z to sigma*(z-lambda)/sqrt(lambda)

                h_0 = expit( deltai +         sigma*(z-lambda)/sqrt(lambda) );
                h_1 = expit( deltai + gamma + sigma*(z-lambda)/sqrt(lambda) );
        ...
        *(S_Delta_C+s) = deltai;
        ...
        for(j=0; j<q_points; j++){
			...
			//Same as previous part, output the changed value, from sigma*z to sigma*(z-lambda)/sqrt(lambda)
			
            *(p_z_lag+j) = (expit( deltai +         sigma*(z-lambda)/sqrt(lambda)) *(1.0-p_z_lag_q))+
                           (expit( deltai + gamma + sigma*(z-lambda)/sqrt(lambda)) *(    p_z_lag_q)) ;
        }
