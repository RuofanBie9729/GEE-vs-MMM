...
#library(devtools)
#install_github("mercaldo/MMLB", force=TRUE) ## install MMLB package
library("MMLB")
library("geepack")
library("geesmv")
library(expm)
...
for(i in 1:R){
nclust = rep(n,N) # create clusters
id     = rep(seq(N), nclust)
trt    =rep(0,N) 
trt[1:(N*p)]=1 # assign treatment group
Xe     = rep(trt, nclust) # binary exposure
time   = as.numeric(sapply( nclust, function(ZZ) {rnorm(ZZ)} )) # normal distributed continuous variable
...
newdata1 = GenBinaryY(mean.formula=~time*Xe, lv.formula=~1, 
          ...
          Yname = "binY") # generate binary response
...
kc.exch <- GEE.var.kc3(data=newdata1, model=as.formula("binY~time+Xe+time:Xe"), betahat=coefficients(gee.fit), phi=1, alpha = as.numeric(summary(gee.fit)$corr[1]))
...
write.csv(GEE_cor_fg, paste0("GEE_fg_sd_K", N, "n",n, ".csv"))
write.csv(GEE_cor_sd, paste0("GEE_correction_sd_K", N, "n",n, ".csv"))
